...
class PengumumanAdapter(val context : Context, val items: List<PostPengumuman>,/*Tambahkan konstruktor untuk detainya */ val listener: (PostPengumuman)-> Unit) : RecyclerView.Adapter<PengumumanAdapter.ViewHolder>(){
    override fun onCreateViewHolder(viewGroup: ViewGroup, position: Int): ViewHolder /*Klik alt enter -> constructor invocation*/{
        val view = LayoutInflater.from(context).inflate(R.layout.list_post_pengumuman,viewGroup,false)
    ...
    class ViewHolder (view:View): RecyclerView.ViewHolder (view){
        ...
        fun bindItem(item: PostPengumuman, listener: (PostPengumuman) -> Unit){/*tambahkan listener yang tadi sudah di buat*/
